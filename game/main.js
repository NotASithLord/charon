...
const _torchDirection = new THREE.Vector3();
...
const RUNGS = [
  { res: [0.85, 1.25], shadowMap: 1024, shadows: true, lights: 14, bloom: 0.5, litePost: false, motes: 36, rag: 48, rifleLights: 12, teamSpots: 3 },
  { res: [0.7, 1.1], shadowMap: 768, shadows: true, lights: 14, bloom: 0.5, litePost: false, motes: 36, rag: 48, rifleLights: 12, teamSpots: 3 },
  { res: [0.7, 1.0], shadowMap: 512, shadows: true, lights: 10, bloom: 0.375, litePost: false, motes: 24, rag: 32, rifleLights: 8, teamSpots: 2 },
  { res: [0.6, 1.0], shadowMap: 512, shadows: false, lights: 8, bloom: 0.375, litePost: true, motes: 24, rag: 24, rifleLights: 5, teamSpots: 1 },
  { res: [0.55, 0.9], shadowMap: 512, shadows: false, lights: 6, bloom: 0.25, litePost: true, motes: 12, rag: 16, rifleLights: 3, teamSpots: 0 },
];
...
const governor = new QualityGovernor({
  ...
  apply: (R, i) => {
    ...
    if (R.shadows) {
      // Shader prewarming changes rungs while live frames are still submitted.
      // Map resolution does not affect a shader variant, so defer this target
      // reallocation until an actual runtime rung change. WebGPU validators
      // otherwise observe the old depth texture being retired in-flight.
      // WebGPU submissions are asynchronous; replacing a depth target from a
      // synchronous quality callback can retire it while the queue still owns
      // the prior frame. Keep one 1024 target on WebGPU and shed the entire
      // shadow pass on lower rungs. WebGL2 can resize safely between frames.
      if (!renderer.backend.isWebGPUBackend && !governor.prewarming
        && torch.shadow.mapSize.x !== R.shadowMap) {
        torch.shadow.mapSize.set(R.shadowMap, R.shadowMap);
        // Three allocates the requested size lazily on the next shadow pass.
        // This only runs on the governor's slow runtime cadence, outside the
        // prewarm loop that cycles several variants in quick succession.
...
function updateRoomLightPool() {
  ...
  const spots = Math.min(RUNGS[rung].teamSpots ?? teamTorches.length, teamTorches.length, lit.length);
