...
assert.match(inviteRoom, /^charon:v2:private:[0-9a-f]{32}$/);
...
assert.equal(quickplayRoom(0), 'charon:quickplay:v2');
assert.equal(quickplayRoom(119_999), 'charon:quickplay:v2');
assert.equal(quickplayRoom(120_000), 'charon:quickplay:v2');
assert.equal(matchScope(['did:b', 'did:a']), matchScope(['did:a', 'did:b', 'did:a']));
assert.notEqual(matchScope(['did:a']), matchScope(['did:b']));
assert.equal(seedForScope('test'), 'charon-multiplayer-v2:test');
assert.equal(matchRoom('match-abc123'), 'charon:v2:match-abc123');
...
assert.equal(validGamePacket({ v: 2, kind: 'state', from: 'did:a', seq: 1 }), true);
assert.equal(validGamePacket({ v: 1, kind: 'state', from: 'did:a', seq: 1 }), false);
assert.equal(validGamePacket({ v: 2, kind: 'eval', from: 'did:a', seq: 1 }), false);
