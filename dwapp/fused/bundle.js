...
var PARAMS = {
  ...
  flamethrower: {
    ...
    player: {
      ...
      fuelPerSec: 8,
      rangeM: 9,
      // the stream reaches this far and stops
      coneDeg: 24,
      // half-angle it bites inside
      ignhitS: 0.1,
      // igniter catch before the stream is live
      tankUnits: 100,
      armoryRefill: 55
      // a spare tank off the rack, not a full swap
    }
  },
...
var ShipGraph = class {
  ...
  *neighbors(nodeIdx, layers, passFn) {
  ...
  hops(from, to, layers, passFn) {
    const c = this._hopsCache ??= /* @__PURE__ */ new Map();
...
function humanDeathToCorpse(sim2, a) {
  ...
  if (a.flamer && a.fuel > 0) {
    corpse.hadFlamer = true;
    corpse.flamerFuel = a.fuel;
  }
  sim2.spawn(corpse);
}
...
function buildNearMesh() {
  ...
  box(-0.29, 1.1, 0, 0.11, 0.34, 0.13, BONE.L_FOREARM, out);
  box(0.29, 1.1, 0, 0.11, 0.34, 0.13, BONE.R_FOREARM, out);
  box(-0.1, 0.72, 0, 0.14, 0.42, 0.16, BONE.L_LEG, out);
  box(0.1, 0.72, 0, 0.14, 0.42, 0.16, BONE.R_LEG, out);
  box(-0.1, 0.26, 0, 0.13, 0.5, 0.14, BONE.L_SHIN, out);
  box(0.1, 0.26, 0, 0.13, 0.5, 0.14, BONE.R_SHIN, out);
  return finish(out);
}
function buildMidMesh() {
  const out = { positions: [], bones: [], indices: [] };
  box(0, 1.3, 0, 0.44, 0.75, 0.24, BONE.TORSO, out);
  box(0, 1.74, 0, 0.22, 0.24, 0.24, BONE.HEAD, out);
  box(-0.1, 0.47, 0, 0.15, 0.94, 0.16, BONE.L_LEG, out);
  box(0.1, 0.47, 0, 0.15, 0.94, 0.16, BONE.R_LEG, out);
  return finish(out);
}
function finish(out) {
  return {
...
var PIVOT = {
  ...
  [BONE.L_LEG]: [-0.1, 0.94, 0],
  [BONE.R_LEG]: [0.1, 0.94, 0],
  [BONE.L_SHIN]: [-0.1, 0.51, 0],
  [BONE.R_SHIN]: [0.1, 0.51, 0]
};
...
function pose(clip, ph) {
  ...
  switch (clip) {
    ...
    case "run": {
      ...
      r[BONE.L_FOREARM] = -0.9;
      r[BONE.R_FOREARM] = -0.9;
    ...
    case "infect_writhe":
      ...
      pitch = 1.2 + s(2, 0.15);
      roll = s(3, 0.4);
      r[BONE.L_ARM] = s(3, 0.9);
      r[BONE.R_ARM] = s(3, 0.9, 1.1);
      r[BONE.L_LEG] = s(2, 0.7, 0.4);
      r[BONE.R_LEG] = s(2, 0.7, 1.9);
      r[BONE.L_SHIN] = Math.abs(s(3, 0.8));
      r[BONE.R_SHIN] = Math.abs(s(3, 0.8, 0.9));
      break;
  }
  ...
  for (const clip of CLIP_DEFS) {
    ...
    for (let f = 0; f < clip.frames; f++) {
      ...
      for (let v = 0; v < mesh.vertexCount; v++) {
        ...
        if (parent !== void 0 && r[parent]) p = rotX(p, PIVOT[parent], r[parent]);
  ...
  render(instances, camera, lodDist = { near: 14, mid: 34 }) {
    ...
    for (const grp of groups) {
      ...
      for (const it of grp) {
        ...
        this.instData[o++] = it.x;
        this.instData[o++] = it.y;
        this.instData[o++] = it.z;
        this.instData[o++] = it.heading;
        this.instData[o++] = it.clip;
        this.instData[o++] = it.animTime;
        this.instData[o++] = it.scale ?? 1;
        this.instData[o++] = it.dead ?? 0;
        this.instData[o++] = it.r;
        this.instData[o++] = it.g;
        this.instData[o++] = it.b;
        this.instData[o++] = 1;
      }
...
var TINTS = {
  ...
  [FACTION.MARINE]: [0.3, 0.56, 0.94],
  [FACTION.INFECTION]: [0.32, 1, 0.42],
  [FACTION.COMBAT]: [0.75, 0.23, 0.16],
  [FACTION.CARRIER]: [0.69, 0.37, 0.85],
  [FACTION.CORPSE]: [0.45, 0.45, 0.45]
};
function bufferToInstances(b, mapPos2 = null) {
  const out = [];
  for (let i = 0; i < b.count; i++) {
    const tint = b.tint[i];
    let x = b.posX[i], y = 0, z = b.posZ[i];
    if (mapPos2) [x, y, z] = mapPos2(b.posX[i], b.posY[i], b.posZ[i]);
...
var sim = new Sim(document.getElementById("seed").value);
...
var mapPos = (x, y, deck) => [
  (x - 640) / 10,
  (5 - deck) * 14,
  // deck 5 at ground, deck 1 highest
...
function frame(now) {
  ...
  const dt = Math.min(0.1, frameMs / 1e3);
  acc += dt;
  let guard = 0;
